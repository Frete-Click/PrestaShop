<div id="box-frete-correios" class="panel panel-info">
  ...
  <div class="panel-body">
    <form name="calcular_frete" id="calcular_frete" action="{$url_shipping_quote}" method="post" />
      ...
      <input type="hidden" name="product-package[0][qtd]" value="{$cart_qtd}" />
      <input type="hidden" name="product-package[0][weight]" value="{$cart_total_weight}" />
      <input type="hidden" name="product-package[0][height]" value="{$cart_total_dimensions['height']}" />
      <input type="hidden" name="product-package[0][width]" value="{$cart_total_dimensions['width']}" />
      <input type="hidden" name="product-package[0][depth]" value="{$cart_total_dimensions['depth']}" />
